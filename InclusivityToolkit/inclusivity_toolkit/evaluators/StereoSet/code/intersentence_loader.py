...
class IntersentenceDataset(Dataset):
        ...
        for example in intersentence_examples:
            ...
            for sentence in example.sentences:
                ...
                if self.tokenizer.__class__.__name__ in [
                    ...
                    input_ids, token_type_ids = self._tokenize(
                        context, sentence.sentence
                    )
                    attention_mask = [1 for _ in input_ids]
                    self.preprocessed.append(
                        (input_ids, token_type_ids, attention_mask, sentence.ID)
                    )
                else:
                    encoded_dict = self.tokenizer.encode_plus(
                        text=context,
                        text_pair=sentence.sentence,
    ...
    def _tokenize(self, context, sentence):
        ...
        context_tokens = [
            self.tokenizer.convert_tokens_to_ids(i) for i in context_tokens
        ]
        ...
        sentence_tokens = [
            self.tokenizer.convert_tokens_to_ids(i) for i in sentence_tokens
        ]

        input_ids = self.add_special_tokens_sequence_pair(
        ...
        position_ids = [
            1 if idx > position_offset else 0 for idx in range(len(input_ids))
        ]
