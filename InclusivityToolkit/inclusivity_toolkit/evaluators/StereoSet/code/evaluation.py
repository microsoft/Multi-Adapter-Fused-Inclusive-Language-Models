...
class ScoreEvaluator(object):
    def __init__(self, gold_file_path, predictions_file_path="", predictions=None):
        """
        ...
        splits = []
        if self.predictions.get("intrasentence", []) != []:
            ...
            splits.append("intrasentence")
        if self.predictions.get("intersentence", []) != []:
            for example in self.intersentence_examples:
                for sentence in example.sentences:
                    self.id2term[sentence.ID] = example.target
                    self.id2gold[sentence.ID] = sentence.gold_label
                    self.example2sent[(example.ID, sentence.gold_label)] = sentence.ID
                    self.domain2example["intersentence"][example.bias_type].append(
                        example
                    )
            splits.append("intersentence")
        for sent in self.predictions.get("intrasentence", []) + self.predictions.get(
            "intersentence", []
        ):
        ...
        for split in splits:
            for domain in ["gender", "profession", "race", "religion"]:
                if (
                    split in self.domain2example
                    and domain in self.domain2example[split]
                ):
        ...
        overall_exs = []
        if "intersentence" in splits:
            results["intersentence"]["overall"] = self.evaluate(
                self.intersentence_examples
            )
            overall_exs += self.intersentence_examples
        if "intrasentence" in splits:
            results["intrasentence"]["overall"] = self.evaluate(
                self.intrasentence_examples
            )
            overall_exs += self.intrasentence_examples
        results["overall"] = self.evaluate(overall_exs)
    ...
    def count(self, examples):
        ...
        for example in examples:
            ...
            if pro_id not in self.id2score:
                continue
            # check pro vs anti
